...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        UIBegin();
        {
            ...
            if (!dictionary) {
                ImGui::TextColored(ImVec4(1, 0, 0, 1), "No active dictionary!");
            }

            if (dictionary && (ImGui::Button("Search") || isEnterPressed)) {
                isEnterPressed = false;
            ...
            ImGui::SameLine();
            if (ImGui::Button("Open dictionary")) {
                fileDialog.Open();
            }
            ...
            if (fileDialog.HasSelected()) {
                dictionary = fopen(fileDialog.GetSelected().string().c_str(), "r");
                searchResults.clear();
